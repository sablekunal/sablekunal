### Hi there, I'm Kunal! 👋

#### *Full-Stack Dev | AI Enthusiast | Aspiring Open Source Contributor*

I am a second-year B.Tech student currently **coding my way out of student debt.** I build web apps that solve real problems and AI agents that can make humans lazy.

---
### 💭 I think
#### If everyone starts contributing to open source. 
![Alt text](https://media1.tenor.com/m/l6Dq9zsAyOkAAAAC/coca-cola-gta.gif)


---

### ❓ What am I up to?

*  **Building Webservice:** Created [**WatchThisMovie**](https://watchthismovie.online) because I got tired of finding a good movie that I actually like.
*  **Automating Chaos:** Building **VESTA** (an AI agent) that fixes raw reviews to well seperated and manages it
*  **Learning:**  App dev(Flutter).
*  **Current Goal:** Crushing **GSoC 2026** and maintaing and improving the existing projects. 

---

### 🛠 The Arsenal (Tech Stack)

* **Languages:** Python , JavaScript , C++ 
* **Web:** React, Next.js, Tailwind CSS 
* **AI/Data:** OpenCV, TensorFlow, Automation Scripts
* **Backend:** Supabase, OAuth, Vercel

---

### 📈 My GitHub Stats

<p align="left">
<img src="https://github-readme-stats.vercel.app/api?username=sablekunal&show_icons=true&theme=radical&hide_border=true" alt="Kunal's Stats" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sablekunal&layout=compact&theme=radical&hide_border=true" alt="Top Languages" />
</p>

---

### ⚡ Fact
I like to build and maintain stuff; you can see the projects I build. Most of them are sudden thoughts that strike, and I start building. 

---

[Website](https://watchthismovie.online) • [Email Me](kunalsable24@gmail.com)
